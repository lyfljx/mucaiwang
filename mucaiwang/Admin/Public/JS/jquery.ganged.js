;(function($){
	...
	function handleData(datas){
		...
		var tempCities = [];
		var tempAreas = [];
		var repclass=/^[0-9]{2}$/;//省的格式:前2位为'01~99'，后4位为'0000'  例：010000
		var reclass=/^[0-9]{4}$/;//城市的格式:前2位为'01~99',中间2位'01~99',后2位为'00' 例:010100
		var reArea=/^[0-9]{6}$/;//区的格式:前2位为'01~99',中间2位'01~99',后2位为'01~99' 例:010101
//		var repclass=/^[0-9]{2}0{4}$/;//省的格式:前2位为'01~99'，后4位为'0000'  例：010000
//		var reclass=/^[0-9]{4}0{2}$/;//城市的格式:前2位为'01~99',中间2位'01~99',后2位为'00' 例:010100
//		var reArea=/^[0-9]{6}$/;//区的格式:前2位为'01~99',中间2位'01~99',后2位为'01~99' 例:010101
		
		//循环取得的数据，添加到用于存放省、市、区的临时数组中
		for(var i=0; i<datas.length; i++){
			tempArr = datas[i].split(':');
			if(repclass.test(tempArr[0])){
				temppclasss.push({'id':tempArr[0],'name':tempArr[1], 'cities':[]});
			}else if(reclass.test(tempArr[0])){
		...
		//将城市信息添加到对应省份中去
		for(var i=0; i<temppclasss.length; i++){
			var pId = temppclasss[i].id.substring(0,2);
			for(var j=0; j<tempCities.length; j++){
				if(tempCities[j].id.substring(0,2) == pId){
					temppclasss[i].cities.push(tempCities[j]);
				}
			}
		}
		...
		pcData.pclasss = temppclasss;
		caData.cities = tempCities;
		
	};
	
	function showpclass(obj, selectPId){
		var tempStr = defaultStr;
		var tempId = defaultId;
		var tempName = defaultName;
		var _pclasss = pcData.pclasss;
		var _cities = [];
		if(_pclasss.length > 0){
			for(var i=0; i<_pclasss.length; i++){
				if(selectPId && selectPId == _pclasss[i].id){
					tempStr += '<a href="javascript:void(0);" val="' + _pclasss[i].id + '" class="selected">' + _pclasss[i].name + '</a>';
					tempId = _pclasss[i].id;
					tempName = _pclasss[i].name;
					_cities = _pclasss[i].cities;
				}else{
					tempStr += '<a href="javascript:void(0);" val="' + _pclasss[i].id + '">' + _pclasss[i].name + '</a>';
				}
			}
		}
		
		$(obj).find('div[name="pclass"]').html('<div class="opts">'+ tempStr +'</div>');
		$(obj).find('div[name="pclass"]').inputbox({'width':currentOpts.width, 'height':currentOpts.height});

		return _cities;
	};
	
	function showclass(obj, cities, selectCId){
		var tempStr = defaultStr;
		var tempId = defaultId;
		var tempName = defaultName;
		...
		$(obj).find('div[name="class"]').html('<div class="opts">'+ tempStr +'</div');
		$(obj).find('div[name="class"]').inputbox({'width':currentOpts.width, 'height':currentOpts.height});
	...
	function createInterval(){
		var spid = $(that).find('input[name="pclass"]').val();
		var scid = $(that).find('input[name="class"]').val();
		var said = $(that).find('input[name="area"]').val();
		var _that = that;
	
		var checkValChange = setInterval(function(){
		
			var _spid = $(_that).find('input[name="pclass"]').val(),
				_scid = $(_that).find('input[name="class"]').val(),
				...
				for(var i=0; i<optsArray.length; i++){
					if(_that == optsArray[i].obj){
						...
						if(optsArray[i].ids.spid != _spid){
							...
							for(var j=0; j< pcData.pclasss.length; j++){
								if(_spid == pcData.pclasss[j].id){
									showclass(_that, pcData.pclasss[j].cities);
									break;
								}
							}
	...
	function init(){
		...
		currentOpts = opts;
		var spid = $(that).find('.pclass').val();
		var scid = $(that).find('.class').val();
		var said = $(that).find('.area').val();
		
		var ps = showpclass(that, spid);
		var cs = showclass(that, ps, scid);
